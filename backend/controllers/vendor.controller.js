...
const createProfile = asyncHandler(async (req, res, next) => {
  ...
  const { clerkId, companyName, gstin, pan, email } = mappedBody;
  
  if (!clerkId || !companyName || !gstin || !pan || !email) {
    return next(ApiError.badRequest('Clerk ID, company name, GSTIN, PAN, and email are required'));
  }

  const existingVendor = await Vendor.findOne({ $or: [{ clerkId }, { email }, { gstin }] });
  if (existingVendor) {
    return next(ApiError.conflict('Vendor with this Clerk ID, email, or GSTIN already exists'));
  }

  // Determine starting status
  const defaultStatus = (clerkId && clerkId.startsWith('mock_vendor_')) ? 'Pending' : 'Draft';
...
const submitRegistration = asyncHandler(async (req, res, next) => {
  ...
  setTimeout(async () => {
    try {
      const updatedVendor = await Vendor.findOne({ clerkId });
      if (updatedVendor && (updatedVendor.status === 'Under Review' || updatedVendor.status === 'Pending Approval')) {
        ...
        console.log(`[SIMULATOR] Auto-approved vendor: ${clerkId} (${updatedVendor.sapVendorCode})`);
      }
    } catch (err) {
