...
const submitBid = asyncHandler(async (req, res, next) => {
  ...
  const { unitPrices, gstRate, freight, deliveryLeadTimeDays, validityDate, remarks } = req.body;
  ...
  const invitation = rfq.invitedVendors.find(v => v.id === vendorId);
  if (!invitation) {
    return next(ApiError.forbidden('Vendor is not invited to participate in this RFQ'));
  }
