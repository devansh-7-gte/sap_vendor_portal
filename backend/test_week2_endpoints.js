...
const MOCK_VENDOR = {
  clerkId: 'mock_vendor_w2_' + Math.floor(Math.random() * 100000),
...
async function runTests() {
  ...
  try {
    ...
    console.log(`➡️ 1. Creating vendor profile for Clerk ID: ${MOCK_VENDOR.clerkId}...`);
    ...
    const rfqPayload = {
      ...
      rfqType: 'AN',
      invitedVendors: [{ id: MOCK_VENDOR.clerkId, name: MOCK_VENDOR.companyName, rating: 92 }],
    ...
    // 3. Submit Bid
    console.log(`➡️ 3. Submitting Bid from ${MOCK_VENDOR.clerkId} to RFQ ${rfqId}...`);
    const bidPayload = {
      unitPrices: {
        '10': 115, // line 10
        '20': 78   // line 20
    ...
    const bidRes = await fetch(`${BACKEND_URL}/rfqs/${rfqId}/bid`, {
      ...
      headers: { 
        ...
        'x-vendor-id': MOCK_VENDOR.clerkId 
      },
    ...
    console.log(`➡️ 5. Awarding RFQ ${rfqId} to ${MOCK_VENDOR.clerkId}...`);
    const awardRes = await fetch(`${BACKEND_URL}/rfqs/${rfqId}/award`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ vendorId: MOCK_VENDOR.clerkId })
    });
