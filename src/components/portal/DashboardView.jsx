      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {isLoading ? (
          ...
          <>
            ...
            <KPICard label="Open POs" value={<span className="tabular-nums">{openPOCount}</span>} icon={ShoppingBag} />

            {/* Stat Card 2: Pending Invoices */}
            <KPICard label="Pending Invoices" value={<span className="tabular-nums">{pendingInvoicesCount}</span>} icon={Receipt} />
            ...
            <KPICard
              ...
              value={
                <span className="tabular-nums">
                  ...
                  <span className="text-[13px] text-text-tertiary font-normal">/100</span>
                </span>
              }
              icon={Activity}
            />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-border">
            ...
            <div className="divide-y divide-border">
              {alerts.slice(0, 2).map((alert, idx) => {
                ...
                return (
                  <div key={idx} className="p-2.5 flex items-start gap-2 hover:bg-surface2 transition-colors duration-150">
                    <div className={`size-5 rounded flex items-center justify-center shrink-0 mt-0.5 border ${alert.iconColor}`}>
                      <Icon className="size-3" />
                    </div>
                    <div className="space-y-0.5">
                      <h5 className="font-bold text-[11px] text-text-primary">{alert.title}</h5>
                      <button
                        onClick={() => setActiveTab(alert.tab)}
                        className="text-text-primary hover:text-emerald-text font-semibold text-[10px] flex items-center gap-0.5 hover:underline pt-0.5 cursor-pointer uppercase tracking-wider transition-colors duration-150"
                      >
                ...
                return (
                  <div key={idx} className="p-2.5 flex items-center justify-between hover:bg-surface2 transition-colors duration-150">
                    <div className="flex items-center gap-2.5 min-w-0">
                      <div className="size-6 rounded bg-surface2 border border-border flex items-center justify-center text-text-secondary shrink-0">
                        <ShoppingBag className="size-3" />
                      </div>
                      <div className="min-w-0">
                        <span className="font-mono font-bold text-text-primary cursor-pointer hover:underline text-[11px]" onClick={() => setActiveTab('pos')}>
                          {row.id}
                        </span>
                        <p className="text-[11px] text-text-tertiary truncate mt-0.5 font-medium">
                    ...
                    <div className="flex items-center gap-3 shrink-0 text-right">
                      <div>
                        <p className="font-mono font-bold text-text-primary text-[11px] tabular-nums">₹{(row.items?.[0]?.netValue || 42500).toLocaleString('en-IN')}.00</p>
                ...
                return (
                  <div key={idx} className="p-2.5 flex items-center justify-between hover:bg-surface2 transition-colors duration-150">
                    <div className="flex items-center gap-2.5 min-w-0">
                      ...
                      <div className="min-w-0">
                        <span className="font-mono font-bold text-text-primary cursor-pointer hover:underline text-[11px]" onClick={() => setActiveTab('invoices')}>
                          {row.invoiceNumber}
                        </span>
                        <p className="text-[11px] text-text-tertiary truncate mt-0.5 font-medium">
                    ...
                    <div className="flex items-center gap-3 shrink-0 text-right">
                      <div>
                        <p className="font-mono font-bold text-text-primary text-[11px] tabular-nums">₹{(row.totalAmount || 52800).toLocaleString('en-IN')}.00</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        ...
        <div className="lg:col-span-7 card overflow-hidden flex flex-col justify-between min-h-[300px]">
          ...
          <div className="p-2.5 bg-surface2/50 border-t border-border grid grid-cols-1 sm:grid-cols-2 gap-2">
            <Button variant="outline" size="sm" onClick={() => setActiveTab('payments')} className="justify-center">
