...
export default function DashboardView({ state, setActiveTab }) {
  ...
  return (
    <div className="space-y-4 max-w-full animate-fade-in pb-16">
      ...
      <div className="bg-primary text-white rounded-sm p-3 shadow-sm border border-primary flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white border border-stone-200 rounded-sm p-2.5 shadow-xs hover:border-stone-300 transition-all flex items-start justify-between">
        ...
        <div className="bg-white border border-stone-200 rounded-sm shadow-xs overflow-hidden flex flex-col justify-between">
          ...
          <div className="divide-y divide-stone-150">
            {[
              ...
              const badgeStyles = row.statusColor === 'blue' ? 'bg-blue-50 text-blue-700 border-blue-200' :
                                  row.statusColor === 'amber' ? 'bg-amber-50 text-amber-700 border-amber-250' :
                                  row.statusColor === 'red' ? 'bg-red-50 text-red-700 border-red-200 animate-pulse' :
                                  'bg-green-50 text-green-700 border-green-200';
              return (
                <div key={idx} className="p-3 flex items-center justify-between hover:bg-stone-50/20 transition-colors">
                  <div className="flex items-center gap-3 min-w-0">
                    <div className={`size-8 rounded-sm flex items-center justify-center border shrink-0 ${
                      row.statusColor === 'blue' ? 'bg-blue-50 border-blue-100 text-blue-700' :
                      row.statusColor === 'amber' ? 'bg-amber-50 border-amber-100 text-amber-700' :
                      row.statusColor === 'red' ? 'bg-red-50 border-red-100 text-red-700' :
                  ...
                  <div className="flex items-center gap-3 shrink-0 text-right">
                    ...
                    <span className={`px-2 py-0.5 rounded-sm text-[8px] font-extrabold border uppercase tracking-wider whitespace-nowrap ${badgeStyles}`}>
                      {row.text}
                    </span>
                  </div>
                </div>
        ...
        <div className="bg-white border border-stone-200 rounded-sm shadow-xs overflow-hidden flex flex-col justify-between">
          ...
          <div className="divide-y divide-stone-150">
            {[
              ...
              const badgeStyles = row.statusColor === 'blue' ? 'bg-blue-50 text-blue-750 border-blue-200' :
                                  row.statusColor === 'red' ? 'bg-red-50 text-red-750 border-red-200 animate-pulse font-bold' :
                                  row.statusColor === 'emerald' ? 'bg-green-50 text-green-700 border-green-200' :
                ...
                <div key={idx} className="p-3 flex items-center justify-between hover:bg-stone-50/20 transition-colors">
                  <div className="flex items-center gap-3 min-w-0">
                    <div className={`size-8 rounded-sm flex items-center justify-center border shrink-0 ${
                      row.statusColor === 'blue' ? 'bg-blue-50 border-blue-100 text-blue-700' :
                      row.statusColor === 'red' ? 'bg-red-50 border-red-100 text-red-700' :
                      row.statusColor === 'emerald' ? 'bg-green-50 border-green-100 text-green-700' :
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[10px] text-stone-500 truncate mt-0.5 font-semibold">
                        Ref PO: <strong className="font-mono">{row.po}</strong>
                      </p>
                    </div>
                  </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="w-full bg-white border border-stone-200 rounded-sm shadow-xs p-3 flex flex-col justify-between min-h-[320px]">
          <div>
              ...
              {[
                ...
                <div key={idx} className="space-y-1.5">
                  ...
                  <div className="w-full bg-stone-100 h-2 rounded-sm border border-stone-200 overflow-hidden">
                    <div
                      className={`h-full rounded-sm transition-all duration-500 ${m.barColor}`}
                      style={{ width: `${m.score}%` }}
                    ></div>
                  </div>
                </div>
          ...
          <div className="mt-6 bg-stone-50 border border-stone-200 p-3 rounded-sm flex items-center justify-between">
            <div>
          ...
          <div>
            ...
            <div className="divide-y divide-stone-150">
              {[
                ...
                <div key={idx} className="p-3 flex items-center justify-between hover:bg-stone-50/20 transition-colors">
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="size-8 rounded-sm bg-emerald-50/50 border border-emerald-100 flex items-center justify-center text-emerald-700 shrink-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[10px] text-stone-500 mt-0.5 font-semibold">
                        Inv: <strong className="font-mono">{row.inv}</strong> &bull; {row.method}
                      </p>
                    </div>
                  </div>
                  
                  <div className="flex items-center gap-5 shrink-0 text-right">
                    <div className="hidden sm:block text-[10px] text-stone-400 font-semibold">
                      <p>Gross: <span className="font-mono text-stone-600">₹{row.gross}</span></p>
                      <p>TDS: <span className="font-mono text-red-650">-₹{row.tds}</span></p>
                    </div>
                    <div>
